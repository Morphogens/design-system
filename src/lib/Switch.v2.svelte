...
export let values: [T, T] = [false as T, true as T]
...
export let value: BinaryWritable<T> = { ...writable(defaultValue), toggle, notValue }
...
<div class="flex flex-row items-center gap-2 cursor-pointer {showLabels !== 'none' ? classes : ''}">
	...
	<div
		class="
		track box-content flex flex-row items-center rounded-3xl h-8 w-12 bg-magenta-30 
		{showLabels === 'none' ? classes : ''}
		"
		on:click={toggle}
	>
